...
const Home = () => {
  ...
  const getCardDataById = () => {
    const filteredData = receivedData.filter((obj) => 
    (parseInt(obj.id)===parseInt(editJobDetailId)));
  ...
  const deleteCardRequest = async () => {
    if(isCardDelete && deleteJobDetailId !== '') {
      const deleteCardId = deleteJobDetailId;
  ...
  return (
    <div className={`${isDarkMode ? 'main-container-dark' : 'main-container'}`}>
      {apiError && (
          <div
            className="bg-red-500 text-white py-2 px-4 rounded-md transition-opacity duration-2000">
            {apiError}
          </div>
        )}
                  ...
                  <div className={`${isDarkMode ? 'job-card-dark' : 'job-card'}`}>
                    <div className='job-header'>
                      ...
                      <div className='job-loc'>
                        ...
                        <p className="job-org">{obj.company_name} - {obj.industry}</p>
                        <p className="job-addr">{obj.location}({obj.remote_type})</p>
                      </div>
                      <div className='job-card-edit-delete-icon-div'>
                        <button ref={editEleRef} id={`edit_card#${obj.id}`} onClick={handleOpenPopup}>
                        <ToolTip message={"Edit job card"} addClass="pointer-events-none absolute bottom-[-40px] left-[-55px] h-auto w-max">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`${isDarkMode ? 'job-card-edit-dark-icon' : 'job-card-edit-icon'}`}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                          </svg>
                        </ToolTip>
                        </button>
                        <button ref={deleteEleRef} id={`delete_card#${obj.id}`} onClick={handleDeleteJobCard}>
                        <ToolTip message={"Delete job card"} addClass="pointer-events-none absolute bottom-[-40px] left-[-58px] h-auto w-max">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`${isDarkMode ? 'job-card-delete-dark-icon' : 'job-card-delete-icon'}`}>
                    ...
                    {obj.apply_type === 'Quick Apply' ? (
                      <div className='w-40 text-white transform translate-x-16 translate-y-2 p-2 float-left'>
                        <input
                          type="button"
                          value="Apply Now"
                          className={`bg-blue-400 border border-grey-500 font-semibold rounded-md shadow-md p-2 ${isDarkMode? 'border-none':''}`}
                        />
                      </div> ) : (
                      <div className='w-auto transform translate-x-16 translate-y-2 text-white p-2 float-left'>
                        <input
                          ...
                          className={`text-blue-500 border border-blue-500 font-semibold rounded-md shadow-md p-2 ${isDarkMode? 'border-2 bg-slate-400':''}`}
                        />
                      </div>)
                    }
                </div>
        ...
        {(isPopupStep1Open || isPopupStep2Open) && <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center"></div>}
        {isPopupStep1Open && <FormPopup isEdit={isCardEdit} isOpen={isPopupStep1Open} onNext={handleOpenPopupStep2} onClose={handleClosePopup} />}
        {isPopupStep2Open && (
        <FormPopupStep2
          isEdit={isCardEdit}
